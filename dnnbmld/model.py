...
class Network(object):
    ...
    def train(self, epoch_num, batch_size, learning_rate, training_data, validation_data,
              beta_fun=lambda x: 0.001, drop=[0,0,0,0,0,0], test_step=None):
        """
        Trains the network.
        Parameters:
        ...
        with self.graph.as_default():
            ...
            for epoch_iter in range(epoch_num): # tqdm(range(epoch_num)):
                self.tot_epochs += 1
                current_beta = beta_fun(self.tot_epochs)
                if epoch_iter%50 == 0:
                    print(str(epoch_iter))
                ...
                for step, data_dict in enumerate(self.gen_batch(training_data, batch_size)):
                    parameter_dict = {self.learning_rate: learning_rate, self.beta: current_beta, self.drop: drop}
#                    feed_dict = dict(data_dict, **parameter_dict)
    ...
    def test(self, data, beta=0):
        ...
        with self.graph.as_default():
            ...
            feed_dict = {**data_dict, **parameter_dict}
            summary = self.session.run(self.all_summaries, feed_dict=feed_dict )
            self.summary_writer.add_summary(summary, global_step=self.tot_epochs)

    def run(self, data, layer, random_epsilon=False, drop=[0,0,0,0,0,0], additional_params={}):
        """
        ...
        with self.graph.as_default():
            ...
            parameter_dict = {self.drop: drop}            
            return self.session.run(layer, feed_dict={**data_dict, **parameter_dict, **additional_params})

    def trav(self, data, layer, drop=0, additional_params={}):
        """
        Run the network and output return the result.
        Params:
        data: Data used for running the network. Same format as training data
        ...
        """
        with self.graph.as_default():
            data_dict = self.gen_data_dict2(data)
            parameter_dict = {self.drop: drop}            
            return self.session.run(layer, feed_dict={**data_dict, **parameter_dict, **additional_params})
    ...
    def from_saved(cls, file_name, change_params={}):
        ...
        """
        with open(io.tf_save_path + file_name + '.pkl', 'rb') as f: # read binary file
    ...
    def graph_setup(self):
        ...
        with self.graph.as_default():
            ...
            self.drop = tf.placeholder(tf.float32, shape=[6], name='drop')
            ...
            with tf.variable_scope('encoder'):
                ...
                for k in range(len(enc_in_dims)):
                    with tf.variable_scope('{}th_enc_layer'.format(k)):
                        ...
                        tdr = tf.reshape(tf.slice( self.drop, [k], [1]),[])
                        temp_layer = tf.nn.dropout(temp_layer, keep_prob = 1-tdr )
            ...
            with tf.variable_scope('decoder'):
                ...
                for k in range(len(dec_in_dims)):
                    with tf.variable_scope('{}th_dec_layer'.format(k)):
                        ...
                        tdr = tf.reshape(tf.slice( self.drop, [k+3], [1]),[])
                        temp_layer = tf.nn.dropout(temp_layer, keep_prob = 1-tdr )
            ...
            with tf.name_scope('recon_loss'):
                ...
                # self.recon_loss = tf.reduce_mean(tf.reduce_sum( tf.nn.sigmoid_cross_entropy_with_logits(logits=self.output, labels=self.labels), axis=1))
            #####################
            # Cost and training #
            #####################
    ...
    def gen_data_dict2(self, data):
        """
        FOR TRAVERSAL CODE
        Params:
        data: same format as training data (see data_loader)
        """
        return {self.mu_sample: data[0],
                self.input2: data[1]}
